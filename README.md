...
1. [setuptools][]: The classic build system. Most powerful, but high learning
   curve and lots of configuration required.
2. [setuptools621][setuptools]: The classic build system, but with the new
   standardized configuration. EXPERIMENTAL.
3. [pybind11][]: This is setuptools but with an C++ extension written in
...
Currently, the best choices are probably flit, PDM, or hatch for pure Python
projects, and setuptools (such as the pybind11 choice) for binary projects.
